...
export class SelectionManager {
	...
	selectUnit(unit: Unit, type: "move" | "attack") {
		...
		if (type === "move") {
			this.currentRange = this.findMovementRange(
				this.currentUnitSelection
			);
	...
	findMovementRange(unit: Unit): PathNodeComponent[] {
		...
		range = range.filter(
			(node) =>
				node.isWalkable && !!(node.walkableMask & unit.player.mask)
		);
		return range;
	}
	...
	findPath(
		...
		if (
			destination &&
			!destination.unit &&
			currentRange.includes(destination.pathNode)
		) {
			const path = this.board.pathFinder.findPath(
				start!,
				destination?.pathNode!,
				this.currentUnitSelection.player.mask,
				currentRange
			);
			return path;
		}
