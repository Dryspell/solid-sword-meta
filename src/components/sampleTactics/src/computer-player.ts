...
export class ComputerPlayer extends Player {
	...
	findClosestCell(unit: Unit, cells: Cell[]) {
		...
		if (closest) {
			...
			for (let cell of cells) {
				const cellDistance = cell.pos.squareDistance(
					unit.cell?.pos ?? ex.vec(0, 0)
				);
	...
	override async makeMove(): Promise<boolean> {
		...
		for (let unit of units) {
			...
			let range: PathNodeComponent[] = [];
			if (unit.cell) {
				range = this.board.pathFinder.getRange(
					unit.cell.pathNode,
					this.mask,
					unit.unitConfig.movement
				);
			...
			if (closestEnemy) {
				...
				const closestCell = this.findClosestCell(
					closestEnemy,
					validCells
				);
				...
				if (!attacked) {
					...
					const currentPath = this.selectionManger.findPath(
						closestCell!,
						range
					);
					...
					await this.selectionManger.selectDestinationAndMove(
						unit,
						closestCell!
					);
					await ex.Util.delay(ENEMY_SPEED);
	...
	lose() {
		this.active = false;
		const computerUnits = this.board
			.getUnits()
			.filter((u) => u.player instanceof ComputerPlayer);
