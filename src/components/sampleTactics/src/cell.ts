...
export class Cell extends ex.Actor {
	...
	set terrain(terrain: Terrain) {
		...
		case Terrain.Grass:
			this.sprite =
					TerrainSpriteSheet.sprites[ex.randomIntInRange(0, 1)];
			break;
		case Terrain.Sand:
			this.sprite =
					TerrainSpriteSheet.sprites[ex.randomIntInRange(2, 3)];
	...
	getDistance(other: Cell) {
		return (
			Math.abs(this.pos.x - other.pos.x) +
			Math.abs(this.pos.y - other.pos.y)
		);
	}
