...
import { SCALE, UNIT_CONFIG, type UnitConfig, type UnitType } from "./config";
...
export class Unit extends ex.Actor {
	...
	onPostDraw(ctx: ex.ExcaliburGraphicsContext) {
		if (this.health > 0) {
			const heart = HeartSpriteSheet.getSprite(
				ex.clamp(this.health, 0, 5),
				0
			);
			if (heart) {
				heart.scale = SCALE;
	...
	async move(path: PathNodeComponent[]) {
		...
		for (let node of pathMinusFirst) {
			...
			const move = new ex.ActionSequence(this, (ctx) => {
				ctx.easeTo(
					currentCell!.pos.sub(this.unitConfig.graphics.offset),
					300,
			...
			const parallel = new ex.ParallelActions([sound, move]);
	...
	getPossibleTargets() {
		if (this.cell) {
			const range = this.cell.board.pathFinder.getRange(
				this.cell.pathNode,
				~this.player.mask,
				this.unitConfig.range
			);
			const cellsWithEnemies = range
	...
	async attack(other: Unit) {
		...
		this.damageManager.spawnDamageNumber(
			other.pos
				.add(other.unitConfig.graphics.offset)
				.add(ex.vec(16 * SCALE.x, 0)),
			this.unitConfig.attack
		);
