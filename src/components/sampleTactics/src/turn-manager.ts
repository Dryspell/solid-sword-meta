...
import { type LevelBase } from "./levels/level-base";
...
export class TurnManager {
	...
	constructor(
		...
		if (players.length === 0)
			throw Error("Players should be non-zero in length");
		...
		// background graphic
		this.turnActor.addChild(
			new ex.Actor({
				color: new ex.Color(240, 50, 50, 0.4),
				width: screenWidth,
				height: 100,
			})
		);
		...
		const failureGroup = new ex.GraphicsGroup({
			members: [
				...
				{
					graphic: failureText1,
					useBounds: false,
					offset: ex.vec(screenWidth / 2, 0),
				},
				{
					graphic: failureText2,
					useBounds: false,
					offset: ex.vec(screenWidth / 2, 100),
				},
			],
		});
	...
	async showVictory() {
		...
		await this.victoryDirections.actions
			.runAction(
				new ex.ParallelActions([
					new ex.ActionSequence(this.victoryDirections, (ctx) =>
						ctx.easeTo(
							this.centerScreen.add(ex.vec(0, 150)),
							transitionTime,
							ex.EasingFunctions.EaseInOutCubic
						)
					),
