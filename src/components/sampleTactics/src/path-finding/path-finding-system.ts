...
export class PathFinder {
	...
	heuristic: (start: PathNodeComponent, end: PathNodeComponent) => number = (
		start: PathNodeComponent,
		end: PathNodeComponent
	) => {
		// manhattan distance
		return (
			Math.abs(start.pos.x - end.pos.x) + Math.abs(end.pos.y - end.pos.y)
		);
	...
	private _getRangeHelper(
		cell: PathNodeComponent,
		accum: PathNodeComponent[],
		mask: number,
		range: number
	) {
		if (range >= 0) {
			...
			cell.connections
				.filter(
					(node) => node.isWalkable && !!(node.walkableMask & mask)
				)
				.forEach((cell) =>
					this._getRangeHelper(cell, accum, mask, range - 1)
				);
		}
	}
	...
	getRange(
		...
		result = result.filter(
			(node, index, nodeArray) => nodeArray.indexOf(node) === index
		);
	...
	findPath(
		...
		mask: number,
		range?: PathNodeComponent[]
	): PathNodeComponent[] {
		const nodes = this.query
			.getEntities()
			.map((n) => n.get(PathNodeComponent)) as PathNodeComponent[];
		nodes.forEach((node) => {
			node.gScore = 0;
			node.hScore = 0;
		...
		start.gScore = 0;
		start.hScore =
			start.gScore + this.heuristic(start, end) * this.heuristicWeight;
		...
		while (openNodes.length > 0) {
			...
			neighbors.forEach((node) => {
				if (openNodes.indexOf(node) === -1) {
					...
					node.hScore =
						node.gScore +
						this.heuristic(node, end) * this.heuristicWeight;
