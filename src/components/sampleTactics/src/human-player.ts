...
export class HumanPlayer extends Player {
	...
	pointerMove(pointer: ex.PointerEvent) {
		...
		// move
		if (this.selectionManager.currentSelectionMode === "move") {
			const currentRange = this.selectionManager.findMovementRange(
				this.selectionManager.currentUnitSelection
			);
			this.selectionManager.showHighlight(currentRange, "range");

			const destination = this.board.getCellByWorldPos(pointer.worldPos);
			if (destination) {
				const currentPath = this.selectionManager.findPath(
					destination,
					currentRange
				);
			...
			const currentRange = this.selectionManager.findAttackRange(
				this.selectionManager.currentUnitSelection
			);
	...
	async maybeMove(unit: Unit, destination: Cell | null) {
		if (destination && unit.canMove()) {
			...
			await this.selectionManager.selectDestinationAndMove(
				unit,
				destination
			);
	...
	async maybeAttack(attacker: Unit, destination: Cell | null) {
		if (
			destination &&
			attacker.canAttack() &&
			this.hasNonPlayerUnit(destination)
		) {
			this.active = false;
	...
	hasNonPlayerUnit(maybeClickedCell: Cell) {
		return (
			maybeClickedCell &&
			maybeClickedCell.unit &&
			maybeClickedCell.unit.player !== this
		);
	}
	...
	hasPlayerUnitWithActions(maybeClickedCell: Cell) {
		return (
			maybeClickedCell?.unit &&
			maybeClickedCell?.unit?.hasActions() &&
			maybeClickedCell.unit.player === this
		);
	}
	...
	lose() {
		this.active = false;
		const playerUnits = this.board
			.getUnits()
			.filter((u) => u.player instanceof HumanPlayer);
